...
def train_fn(TIMESTAMP, CONF):
    ...
    #                                 num_classes=CONF['model']['num_classes'], ###
        ...
        num_folds = len([x for x in os.listdir(paths.get_ts_splits_dir()) if 'Fold-' in x])
        
        #Create folds directories
        utils.create_folds_tree(num_folds)
        for fold_iter in range(0,num_folds):
            # Load the training data
            X_train, y_train = load_data_splits(splits_dir=paths.get_ts_splits_dir(),
                                                im_dir=paths.get_images_dir(),
                                                split_name=f'Fold-{fold_iter}/train')
        ...
        #                               num_classes=CONF['model']['num_classes'], ###
            ...
            history = model.fit_generator(generator=train_gen,
                                      ...
                                      validation_data=val_gen,
                                      validation_steps=val_steps,
                                      callbacks=utils.get_callbacks(CONF, fold_iter=fold_iter),
                                      verbose=1, max_queue_size=5, workers=4,
                                      use_multiprocessing=CONF['training']['use_multiprocessing'],
                                      initial_epoch=0)

            # Saving everything
            print('Saving data to {} folder.'.format(f'{paths.get_timestamped_dir()}/Fold-{fold_iter}'))
            print('Saving training stats ...')
            stats = {'epoch': history.epoch,
                     'training time (s)': round(time.time()-t0, 2),
                     'timestamp': TIMESTAMP,
            ...
            stats = json_friendly(stats)
            stats_dir = paths.get_stats_dir()
            with open(os.path.join(stats_dir, f'stats_Fold-{fold_iter}.json'), 'w') as outfile:
                json.dump(stats, outfile, sort_keys=True, indent=4)
